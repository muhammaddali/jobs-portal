...
export const JobCard = ({ company, logo, New, Featured, position, role, level, postedAt, contract, location, languages = [], tools = [], onClickFilter }) => {
    return (
        <>
                ...
                >
                    ...
                    {languages.map((lang, index) => (
                        <span
                            ...
                            key={index} className="badgeBg badgeText px-3 py-1 rounded text-sm font-bold badgeHover"
                        >
                            {lang}
                        </span>
                    ))}
                    {tools.map((tool, index) => (
                        <span
                            ...
                            key={index} className="badgeBg badgeText px-3 py-1 rounded-sm text-sm font-bold badgeHover"
                        >
                            {tool}
                        </span>
                    ))}
